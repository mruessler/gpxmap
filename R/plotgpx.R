...
plotgpx <- function(folder, type = "osm", zoom = 14, col = "purple", mapborder = 0.1, pdftitle = "Map plot") {
	# load the gpx data
	gpx <- loadmeerun(folder = folder)
	# set map boundaries
	lon <- grDevices::extendrange(c(min(gpx$longitude), max(gpx$longitude)), f = mapborder)
	lat <- grDevices::extendrange(c(min(gpx$latitude), max(gpx$latitude)), f = mapborder)
	...
	# create a pdf file
	grDevices::pdf(file = "Map.pdf", title = pdftitle, width = 19.69, height = 27.95)
	ggmap::ggmap(map, extent = "device") +
		ggplot2::geom_path(ggplot2::aes(x = longitude, y = latitude, group = factor(track)),
